  ...
  if (name === TAG_NAME) {
    ...
    for (let i = 0, l = childNodes.length; i < l; i++) {
      ...
      if (isElementNode(childNode)) {
        const result: ILinesOrNull = transpileReactiveHTMLRXSwitchCaseToReactiveDOMJSLines(
          childNode,
          SWITCH_MAP_NAME,
        ...
        if (result === null) {
          const result: ILinesOrNull = transpileReactiveHTMLRXSwitchDefaultToReactiveDOMJSLines(
            childNode,
            SWITCH_DEFAULT_NAME,
            requireExternalFunction,
          );
