...
import { REQUIRE_CREATE_REACTIVE_ASYNC_NODE_CONSTANT } from './types/require-create-reactive-async-node.type';
...
export function generateDefaultRequireExternalFunction<GKey extends string, GReturn>(
  ...
  const requireExternalFunction: IRequireExternalFunction<GKey> = (name: GKey): string => {
    switch (name) {
      ...
      case REQUIRE_SET_REACTIVE_PROPERTY_CONSTANT:
        return add('setCaseInsensitiveReactiveProperty');
      ...
      // reactive nodes
      case REQUIRE_CREATE_REACTIVE_ASYNC_NODE_CONSTANT:
        return add('createReactiveAsyncNode');
